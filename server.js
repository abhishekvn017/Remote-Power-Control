...
async function executeSSHCommand(device, command, retries = 2) {
    ...
    for (let attempt = 1; attempt <= retries; attempt++) {
        ...
        try {
            ...
            // Add error handler to the underlying connection to catch ECONNRESET
            // This is critical for power commands where the remote closes the connection
            if (ssh.connection) {
                ssh.connection.on('error', (err) => {
                    // Silently ignore ECONNRESET - expected when device shuts down/sleeps
                    if (err.code === 'ECONNRESET' || err.message.includes('ECONNRESET')) {
                        console.log('SSH connection reset (expected for power commands)');
                    } else {
            ...
            // Use try-catch for dispose to handle already-closed connections
